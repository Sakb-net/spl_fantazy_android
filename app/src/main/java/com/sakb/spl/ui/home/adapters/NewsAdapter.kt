...
class NewsAdapter : ListAdapter<HomeResponse.New, NewsAdapter.FixturesViewHolder>(NewsDiffCallback()) {
    ...
    class FixturesViewHolder(
        ...
        fun bind(data: HomeResponse.New) = data.run {
            ...
            Glide.with(binding.imageViewThumbNews).load(Constants.baseUrl + image)
                .override(150).diskCacheStrategy(DiskCacheStrategy.NONE).into(binding.imageViewThumbNews)
        }
    }
